...
int main() {
	...
	stringstream buf;
	img2.savePfm(buf);
	string bufString{buf.str()}; //buf to string
	...
	};	// Reference PFM file content
	unsigned int referenceLength = 84;
	string refString{reinterpret_cast<char*>(referencePfm), referenceLength};//ref to string

	assert(bufString.compare(refString)==0);
